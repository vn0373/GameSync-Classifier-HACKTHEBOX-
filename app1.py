...
loaded_model = load_model("C:/Users/VISHAL KHUMAR P D/Downloads/HacktheBox Challenge/Final_model.h5")
...
def main():  
    ...
    if uploaded_file is not None:
        ...
        with open(os.path.join("C:Users/VISHAL KHUMAR P D/Downloads/HacktheBox Challenge/Temp",uploaded_file.name.split("/")[-1]),"wb") as f:
            f.write(uploaded_file.getbuffer())
        st.success("File Uploaded Successfully")

        if st.button('Apply Deep learning classification'):
            output_file_path = os.path.join("C:Users/VISHAL KHUMAR P D/Downloads/HacktheBox Challenge/VideoSample/",
                                            uploaded_file.name.split("/")[-1].split(".")[0] + "_output1.mp4")
            with st.spinner('Please wait...Your video is processing.'):
                predict_on_video("C:Users/VISHAL KHUMAR P D/Downloads/HacktheBox Challenge/Temp/" +
                ...
                os.chdir('C:Users/VISHAL KHUMAR P D/Downloads/HacktheBox Challenge/VideoSample/')
                subprocess.call(['ffmpeg','-y', '-i', uploaded_file.name.split("/")[-1].split(".")[0]+".mp4",'-vcodec','libx264','-f','mp4','output4.mp4'],shell=True)
                st.success('Done!')

                video_path = os.path.join("C:Users/VISHAL KHUMAR P D/Downloads/HacktheBox Challenge/VideoSample/" + uploaded_file.name.split("/")[-1].split(".")[0]+"_output1.mp4")
